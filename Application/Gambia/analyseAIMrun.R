...
# to include the read.annotated.nexus function from OutbreakTools
source('~/Documents/github/IsolationWithMigration/Application/Gambia/annotatedTreeReader.R')
...
for (a in seq(2,2)){ #loci
  for (b in seq(1,2)){ # migration priors
    for (c in seq(1,1)){ # which chromosomes
      tree_files <- list.files(path=paste("./combined/", loci[[a]], sep=""), pattern=paste("*",chr[[c]],"_",loci[[a]], "_sub[123]_", mig[[b]] ,"_species.trees", sep=""), full.names = TRUE)
      ...
      for (tf in seq(1,length(tree_files))){
        ...
        for (i in seq(1, plot_nr)){
          ...
          system(paste("/Applications/BEAST\\ 2.6.0/bin/logcombiner -renumber -burnin 0 -log", fname1, "-o", fname2, sep=" "))
        ...
        for (i in seq(1, plot_nr)){
          ...
          print((i-1)*3 + tf)
          plot_all_sub[[(i-1)*3 + tf]] <- p.sub
          
          plot_all_collapse[[(i-1)*3 + tf]] <- p.collapse
          

      

          # fname1 <- gsub("\\.trees", paste("_", i, "_", counts[i], "all.pdf", sep=""), trees)
          # ggsave(plot=p, filename=fname1,width=3, height=3)
          # 
          # fname1 <- gsub("\\.trees", paste("_", i, "_", counts[i], "sub.pdf", sep=""), trees)
          # ggsave(plot=p.sub, filename=fname1,width=3, height=3)
      ...
      require(gridExtra)
      require(grid)
      # p_all <- do.call("grid.arrange",c(plot_all, ncol=3))
      p_all_sub <- do.call("grid.arrange",c(plot_all_sub, ncol=3))
      p_all_collaps <- do.call("grid.arrange",c(plot_all_collapse, ncol=3))
      
      if (plot_how_many_trees==2){
        ggsave(plot=p_all_sub,paste("/Users/nmueller/Documents/github/IsolationWithMigration/Figures/Gambia/", chr[[c]], "_", loci[[a]], "_", mig[[b]], "_sub.pdf", sep=""),width=8, height=6)
        ggsave(plot=p_all_collaps,paste("/Users/nmueller/Documents/github/IsolationWithMigration/Figures/Gambia/", chr[[c]], "_", loci[[a]], "_", mig[[b]], "_collapse.pdf", sep=""),width=8, height=6)
      }else{
        ggsave(plot=p_all_sub,paste("/Users/nmueller/Documents/github/IsolationWithMigration/Figures/Gambia/", chr[[c]], "_", loci[[a]], "_", mig[[b]], "_sub.pdf", sep=""),width=8, height=3)
        ggsave(plot=p_all_collaps,paste("/Users/nmueller/Documents/github/IsolationWithMigration/Figures/Gambia/", chr[[c]], "_", loci[[a]], "_", mig[[b]], "_collapse.pdf", sep=""),width=8, height=3)
      }
    }
  }
}
