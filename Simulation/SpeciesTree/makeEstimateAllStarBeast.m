...
system('rm -r StarBeast');
system('mkdir StarBeast');
...
for i = 1 : length(bpp_files)
    ...
    for j = 1 : length(use_genes)
        ...
        folder = [pwd '/StarBeast'];
        filename = strrep(bpp_files(i).name, '.txt', '');
        filename = strrep(filename, 'BPP', ['StarBeast_' num2str(use_genes(j)) 'Genes']);
        
        % get the migration and Ne to make the prior
        tmp = strsplit(bpp_files(i).name, '_');      
        % create xml
        makeStarBeastxml(folder,filename, 500000000, 500000,...
            genes, species, [], speciesTree,...
            true, true, true,...
            true, use_genes(j), false);
